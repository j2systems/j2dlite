...
function controlContainer(dothis)
{
	...
	switch (thisaction) {
		...
		case "cachecon":
			...
			break;
		case "cachesmp":
			var URL = thiscontainer + ":57772/csp/sys/UtilHome.csp"
...
/////////////////CREATE OPERATIONAL ENVIRONMENT
//
// 1. Create button row
//
		...
		case "COMPLETE":
			...
			switch (thisAction) {
				case "stop":
					...
					break;
				case "start":
					//replace thisContainer with stop stuff
					doSend("containerinfo=" + thisContainer);
					break;
				case "commit":
					//set commit to normal. write status message;
...
function addButtons(containerName,running,isHS)
{
	if (running == "offline") {
		newButtons = ["Start","Export","Commit","Delete"];
	}else {
		if (isHS == "true") {
			newButtons = ["Stop","Console","CacheCon","CacheRtn","ExecScript"];
		}else {
			newButtons = ["Stop","Console","ExecScript"]
		}
	}
