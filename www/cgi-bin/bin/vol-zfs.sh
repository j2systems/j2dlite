...
echo "ZFSVOL,RELOAD,Volume,Size,Type,Reference"
COUNT=0
...
do
	...
	DOCKERREF=$(echo ${VOLLIST}|cut -d "," -f1)
		...
		echo "ZFSVOL,${COUNT},${NAME},${USED},${TYPE},(${DOCKERREF}) ${CROSSREF}"
	else
		if [[ $(echo "${THISZFSVOL}"|grep -c "directIO") -eq 1 ]]
		then
			COUNT=$((++COUNT))
			echo "ZFSVOL,${COUNT},${NAME},${USED},volume,XFS for direct io"

		# native docker volume || "${THISZFSVOL}" == "docker" ]]
		else
			COUNT=$((++COUNT))
			echo "ZFSVOL,${COUNT},${NAME},${USED},no ref,no ref"
			#zfs destroy ${NAME} -r
		fi
